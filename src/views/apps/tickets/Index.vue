				...
				>
					<b-card-body class="text-center">
						...
						<b-card-text class="mb-2">
							or choose a category to quickly find the tickets
							that may be raised
						</b-card-text>
			...
			<section id="faq-tabs">
				<b-card-text class="text-center py-1 font-weight-bold h5">
					<b-alert
						variant="success"
						v-if="seachQuery"
						class="text-center py-1 font-weight-bold h5"
						show
					>
						Searched Result
					</b-alert>
				...
				>
					...
					<template #tabs-end>
						<b-img
							fluid
							:src="
								require('@/assets/images/faq-illustrations.svg')
							"
...
<script>
	...
	export default {
		...
		computed: {
			computeTickets() {
				let tickets = this.seachQuery
					? this.searchQueryResult
					: this.$store.getters["Ticket/allTicketGetter"];
		...
		methods: {
			structureItemForSearch(tabIndex) {
				let obj = {
					0: { field: "category", value: "payment" },
					1: { field: "category", value: "confirmation" },
					2: { field: "category", value: "app usablility" },
					3: { field: "category", value: "others" },
					4: { field: "category", value: "administration" },
				};
				return obj[tabIndex];
			},
			fetchTickets(item) {
				...
				this.isSending = true;
				this.$store
					.dispatch("Ticket/GET_TICKETS", payload)
					.then((resp) => {
